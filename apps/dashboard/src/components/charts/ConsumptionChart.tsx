...
import type { ConsumptionPoint, TimeRange } from '../../domain/models'
import { useI18n } from '../../i18n/I18nContext'
import { formatDate, formatDateTime, formatDayMonth, formatHourMinute } from '../../lib/format'
import { AXIS_TICK, CHART_INK, SERIES } from './theme'
import { ChartTooltip } from './ChartTooltip'

export function ConsumptionChart({ data, range }: { data: ConsumptionPoint[]; range: TimeRange }) {
  const { t } = useI18n()
    ...
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }} barCategoryGap="20%">
        ...
        <XAxis
          ...
          minTickGap={32}
        />
        <YAxis tick={AXIS_TICK} tickLine={false} axisLine={false} width={44} unit=" kWh" />
        <Tooltip
          cursor={{ fill: 'rgba(11, 11, 11, 0.04)' }}
          content={
            <ChartTooltip
              labelText={hourly ? formatDateTime : formatDate}
              valueText={(v) => `${v.toFixed(2)} kWh`}
            />
          }
        />
        <Bar
