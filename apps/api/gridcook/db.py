"""In-memory SQLite loader for the Oloika synthetic dataset.

The documented source of truth is the CSV/JSON files in ``data/synthetic``
(see ``docs/oloika_data_schema_and_prediction_notes.md``). On startup the API
loads those files into an in-memory SQLite database so endpoints can filter,
order, and paginate with plain SQL without any external database service.
"""
...
# Runtime tables are created empty and populated by write endpoints (e.g. a
...
_RUNTIME_TABLES = (
    ...
    CREATE TABLE IF NOT EXISTS cooking_plans (
        ...
        suggested_credit_gain INTEGER NOT NULL,
...
def get_connection() -> sqlite3.Connection:
    """Return the shared in-memory connection, loading the dataset on first use."""
    global _connection
